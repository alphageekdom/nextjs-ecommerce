...
export async function approvePayPalOrder(
  ...
  try {
    ...
    if (
      !captureData ||
      captureData.id !==
        ((order.paymentResult as PaymentResult)?.id ||
          captureData.status !== "COMPLETED")
    ) {
      throw new Error("Error with PayPal payment");
    }
...
export async function updateOrderToPaidCOD(orderId: string) {
  try {
    ...
    return { success: true, message: "Order COD marked as paid" };
  } catch (error) {
    return { success: false, message: formatError(error) };
  }
}
...
export async function deliverOrder(orderId: string) {
  try {
    ...
    if (!order.isPaid)
      throw new Error("Order cannot be delivered as payment is pending");
